import type { PivotRoute, ProfileContext, SimulationResult, SupplierAfterSnapshot, SupplierSnapshot } from "./simulation-types";
...
function contextBlock(event: string, ctx: ProfileContext): string {
...
export function buildStreamingSimulationPrompt(event: string, ctx: ProfileContext): string {
  return `${contextBlock(event, ctx)}
...
function parsePivot(o: Record<string, unknown>): PivotRoute | null {
  const h = o.horizon;
